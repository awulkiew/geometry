...
operator&&(Pred1 const& p1, Pred2 const& p2)
{
    ...
    return std::pair<Pred1, Pred2>(p1, p2);
}

template <typename Tuple, typename Pred> inline
geometry::tuples::push_back_t<Tuple, Pred>
operator&&(Tuple const& t, Pred const& p)
{
    //typedef std::conditional_t<is_predicate<Pred>::value, Pred, Pred const&> stored;
    
    return geometry::tuples::push_back(t, p);
}
